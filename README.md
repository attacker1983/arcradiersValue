...
- Run `node scripts/test-lookup.js "<item name>"` to exercise the scraping logic against arctracker.io for any item name.
- Offline fixtures keep the test usable without network access:
  - `wolfpack` resolves to a 5,000 credit value from the bundled fixture.
  - `adrenaline shot` resolves to a 1,200 credit value from the bundled fixture.
  - Other names will attempt a live fetch first and only succeed offline if you add a matching fixture under `scripts/fixtures/`.
